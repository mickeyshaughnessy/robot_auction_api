...
def has_sufficient_funds(data):
    ...
    outstanding_bids = [
        json.loads(b) for _, b in redis.hscan_iter("REDHASH_ALL_LIVE_BIDS")
        if json.loads(b).get("bidder_account_id") == account_id
    ]
    
    total_outstanding = sum(b.get("bid_price", 0) for b in outstanding_bids)
    return account.get("balance", 0) - total_outstanding + bid_price > 0
